...
class MicrobiomeDataset:
    ...
    def target_names(self) -> list:
        ...
        """
        return (
            self.metadata[self.target_column].astype('category').cat.categories.tolist()
        )
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        info = {
            ...
            'class_distribution': self.metadata[self.target_column]
            .value_counts()
            .to_dict(),
    ...
    def filter_features(
        ...
        if inplace:
            ...
            new_dataset._preprocessing_history = self._preprocessing_history + [
                step_info
            ]
            return new_dataset

    def clr_transform(
        self, log_n0: float = 1e-6, inplace: bool = True
    ) -> Optional['MicrobiomeDataset']:
        ...
        # Track preprocessing step
        step_info = {'step': 'clr_transform', 'parameters': {'log_n0': log_n0}}

        if inplace:
            self.abundance = (
                clr_data.T
            )  # clr_transform returns samples x features, we want features x samples
            self._preprocessing_history.append(step_info)
            return None
        else:
            # Create new instance
    ...
    def _validate_data(self) -> None:
        ...
        if self.target_column not in self.metadata.columns:
            raise ValueError(
                f"Target column '{self.target_column}' not found in metadata"
            )

        # Check if sample column exists
        if self.sample_column not in self.metadata.columns:
            raise ValueError(
                f"Sample column '{self.sample_column}' not found in metadata"
            )
        ...
        if not abundance_samples.issubset(metadata_samples):
            ...
            print(
                f"Warning: {len(missing_in_metadata)} samples in abundance data not found in metadata"
            )
