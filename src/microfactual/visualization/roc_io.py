"""File-saving ROC helpers used by the CLI pipeline.

For interactive/figure-returning plots use
:func:`microfactual.visualization.plots.plot_roc`.
"""
...
def plot_roc_to_path(
    y_true: pd.Series,
    y_probs: pd.Series,
    save_path: str | None = None,
    show: bool = False,
) -> None:
    """Plot a ROC curve and optionally save it to ``save_path`` or display it.
...
def save_roc_curve(
    ...
    """Plot and save the ROC curve to ``output_dir``.
    ...
    plot_roc_to_path(labels, probs, save_path=output_dir, show=False)
